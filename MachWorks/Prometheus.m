...
fixed_input.L_fuselage = 3; % m -> FA18 fuselage length
...
geom.empty_weight = lb2N(100); % Gotta be Newtons m8. This drives MTOW using historical relations which eventually informs the amount of fuel which can be carried
geom.Lambda_LE = 29.3; % deg - Leading Edge Sweep
geom.c_r = 0.7; % m - Root Chord
geom.c_t = 0.1; % m - Tip Chord
geom.span = 1.5; % m - Wing Span
geom.W_F = lb2N(20); % N - Fixed Weight (Avionics)
geom.engine = "H20"; % engine: A string code which you can see in engine_lookup.xslx. More info in engine_getData

%% Make the prom object
%                                     empty_weight,       Lambda_LE,     c_r,       c_t,    span,    num_engine,      engine,      W_F
prom = planeObj(fixed_input, "Prometheus", geom.empty_weight, geom.Lambda_LE, geom.c_r, geom.c_t, geom.span,    2,         geom.engine, geom.W_F);
prom = prom.applyLoadout(empty); % Just two sidewinders
...
fprintf("The Prometheus has a unit cost of %.2f million dollars and a stall speed of %.2f kt (lands at %.2f kt) for MTOW", prom.calcUnitCost(), ms2kt( prom.calcStallSpeed(0, prom.MTOW) ), ms2kt( prom.calcLandingSpeed(0, prom.MTOW) ) );

[climbRate, climbAngle, climbSpeed] = prom.calcMaxClimbRate(0, prom.MTOW, 1);
...
[turn_rate, n] = prom.getMaxTurn(0, 0.5, prom.MTOW);
fprintf("\nSealevel, Mach 0.5 max turn rate = %.2f deg/s at a load factor of %.2f", turn_rate, n)

fprintf("\nPrometheus spot factor = %.3f (Projected area of %.3f m2)", prom.calcSpotFactor(0.3193), prom.calcFoldedWingProjection(0.3193) )

[maxAlt, maxAltMach, excessPower] = prom.calcMaxAlt(prom.MTOW, 1);
fprintf("\nThe prom has a service ceiling of %.2f kf (does Mach %.2f at its ceiling with a CL of %.3f).", m2ft(maxAlt)/1000, maxAltMach, prom.calcTrimCL(maxAlt, maxAltMach, prom.MTOW))

[maxMach, maxMachAlt] = prom.calcMaxMach(prom.MTOW, 1);
fprintf("\nPrometheus has a maximum mach number of %.3f which it reaches at %.2f kf", maxMach, m2ft(maxMachAlt)/1000)

[h_maxR, M_maxR, V_maxR, L2D_maxR] = prom.findMaxRangeState(prom.MTOW);
[h_maxE, M_maxE, V_maxE, LD_maxE] = prom.findMaxEnduranceState(prom.MTOW);
