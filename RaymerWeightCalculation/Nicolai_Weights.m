...
%------------------ read from struct:---------------------------------
W_TO = 66000;          % takeoff weight [lb]
M0   = 1.1;            % max Mach number at sea level
q    = 1800;            % max dynamic pressure [lb/ft^2]
L = m2ft(17.54);                % fuselage length [ft]
H = 7;                 % maximum fuselage height [ft]
W_TRON = 2500;   % avionics [lb]
% Engine data
N_E   = 2;             % number of engines
W_ENG = 2445;          % bare engine weight per engine [lb]
...
M_D   = 1.62;        % design Mach number = max Mach number
...
W_landing_retardation = 250; % [lb] estimate comes from Nicolai Figure 10.10
composite_percentage=20; % F-18E is 20%, F-35 is 35%
