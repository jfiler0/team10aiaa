function cost_struct = xanderscript_modified(geom, do_tables, do_figures)
    ...
    W_ampr=N2lb(geom.weights.empty.v); %lb (kinda like empty weight)
    ...
    W_F_used=0.8*(W_TO-W_ampr); % fuel used in lbs per mission
    ...
    cost_struct.RDTE.airframe_engineering=MHR_aed_r*R_e_r; % inflation accounted for in R_E_r
    
    % Sec 3.2 Dev Support and Testing Cost
    cost_struct.RDTE.development_support_testing=0.008325*W_ampr^0.873*V_max^1.890*N_rdte^0.346*CER(2025)*F_diff;
    
    % Sec 3.3 Flight Test Airplanes Cost
    C_avionics=0.3/0.7*(51.813e6); % TODO: update per iteration to get 30% of AEP
    cost_struct.RDTE.flight_test.engines_avionics = (C_e_r*N_e+C_p_r*N_p+C_avionics)*(N_rdte-N_st);
    MHR_man_r=28.984*W_ampr^0.74*V_max^0.543*N_rdte^0.524*F_diff; % manufacturing manhours
    cost_struct.RDTE.flight_test.manufacturing_labor=MHR_man_r*R_m_r; %manufacturing labor cost
    cost_struct.RDTE.flight_test.materials   = 37.632*F_mat*W_ampr^0.689*V_max^0.624*N_rdte^0.792*CER(2025); %material costs
    MHR_tool_r=4.0127*W_ampr^0.764*V_max^0.899*N_rdte^0.178*N_r_r^0.066*F_diff;
    cost_struct.RDTE.flight_test.tooling = MHR_tool_r * R_t_r; % tooling costs based on manufacturing hours and rate
    cost_struct.RDTE.flight_test.quality_control= 0.13*cost_struct.RDTE.flight_test.manufacturing_labor; %quality control costs
    C_fta_r=cost_struct.RDTE.flight_test.engines_avionics+cost_struct.RDTE.flight_test.manufacturing_labor+cost_struct.RDTE.flight_test.materials+cost_struct.RDTE.flight_test.tooling+cost_struct.RDTE.flight_test.quality_control;
    
    % Sec 3.4 Flight Test Operations Cost
    cost_struct.RDTE.flight_test_operations=0.001244*W_ampr^1.16*V_max^1.371*(N_rdte-N_st)^1.281*CER(2025)*F_diff*F_obs;
    ...
    cost_struct.RDTE.software_development=cost_software*CER(2025)/CER(2013); % inflation
    
    % Final RDTE
    C_RDTE=(cost_struct.RDTE.airframe_engineering+cost_struct.RDTE.development_support_testing+C_fta_r+cost_struct.RDTE.flight_test_operations+C_fin_r+cost_struct.RDTE.software_development)/(1-F_tsf-F_pro_r);
    cost_struct.RDTE.test_sim_facilitis=C_RDTE*F_tsf; % accounts for sec 3.5
    cost_struct.RDTE.profit=C_RDTE*F_pro_r; % accounts for RDTE profit
    ...
    cost_struct.production.airframe_engineering= MHR_aed_program*R_e_m-cost_struct.RDTE.airframe_engineering;
    ...
    cost_struct.production.production.engines_avionics=(C_e_m*N_e+C_p_m*N_p+C_avionics_m)*N_m;
    ...
    cost_struct.production.production.manufacturing_labor=MHR_man_program*R_m_m-cost_struct.RDTE.flight_test.manufacturing_labor;
    C_mat_program=37.632*F_mat*W_ampr^0.689*V_max^0.624*N_program^0.792*CER(2025);
    cost_struct.production.production.materials=C_mat_program-cost_struct.RDTE.flight_test.materials;
    R_t_m=R_t_r;
    MHR_tool_program=4.0127*W_ampr^0.764*V_max^0.899*N_program^0.178*N_r_m^0.066*F_diff;
    cost_struct.production.production.tooling=MHR_tool_program*R_t_m-cost_struct.RDTE.flight_test.tooling;
    cost_struct.production.production.quality_control=0.13*cost_struct.production.production.manufacturing_labor;
    C_apc_m = cost_struct.production.production.engines_avionics + C_int_m + cost_struct.production.production.manufacturing_labor + cost_struct.production.production.materials + cost_struct.production.production.tooling + cost_struct.production.production.quality_control;
    ...
    C_MAN=cost_struct.production.airframe_engineering+C_apc_m+cost_struct.production.flight_test_operations+C_fin_m;
    ...
    cost_struct.operations.personnel.crew=N_serv*N_crew*R_cr*Pay_crew*OHR_crew*N_yr; % crew 
    R_m_ml=45*CER(2025)/CER(1989); % maintance labor rate
    MHR_flthr= 20; % maintance hours per flight hour (15-35 for fighters)
    
    cost_struct.operations.personnel.maintenance=N_serv*N_yr*U_ann_flt*MHR_flthr*R_m_ml; % direct maintaince peeps
    C_PERSDIR=cost_struct.operations.personnel.crew+cost_struct.operations.personnel.maintenance; 
    ...
    C_OPS=(cost_struct.operations.fuel_oil+C_PERSDIR+cost_struct.operations.consumables+cost_struct.operations.misc)/(1-f_persind-f_spares-f_depot);
    ...
    cost_struct.disposal = cost_struct.disposal;
    cost_struct.total = cost_struct.RDTE.total + cost_struct.production.total + cost_struct.operations.total + cost_struct.disposal;
