...
% Prop model needs to output: [TA, TSFC, alpha]
    ...
    theta = in.cond.T.v/T_SL; delta = in.cond.P.v/P_SL; 
    
    theta_0 = theta * (1 + (gamma-1)/2 * in.cond.M.v^2);
    delta_0 = delta * (1 + (gamma-1)/2 * in.cond.M.v^2)^(gamma/(gamma-1));
    ...
    F_th_mil = in.geom.prop.T0_NoAB.v * alpha_dry; % (whatever unit thrust was passed with)
    F_th_AB = in.geom.prop.T0_AB.v * alpha_AB; % (whatever unit thrust was passed with)
    
    TSFC_mil = (0.9 + 0.30 * in.cond.M.v) * sqrt(theta); %hour^-1; Mattingly Eq.3.55a (No, these are lbm/lbf*hr)
    ...
    alpha = max(alpha_dry + in.cond.ab_throttle.v * (alpha_AB - alpha_dry), 0);
    if(alpha < 0)
        TA = 0;
    end
