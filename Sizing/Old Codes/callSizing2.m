...
while(i < iter_limit)
    i = i + 1;
    W0_new = W0_next(W0, mission, plane, W_TO_O_S, debug);
    if(abs(W0_new - W0)/W0 < eps)
        W0 = W0_new;
        % fprintf("\nBreak on i = %.0f", i);
        break;
    end
...
if(i == iter_limit)
    warning("W0 did not converge.")
end

function W0_new = W0_next(W0_guess, mission, plane, W_TO_O_S, debug)
    if W0_guess <= 0
        error("W0_guess is less than 0")
    end
    if isnan(W0_guess)
        error("W0_guess became NaN")
    end
    ...
    step_size = 20; % In ratio of increase/decrease; 0.5 = 50%

    % Fall back for when the guessed weight is too low
    if(W0_new < 0)
        W0_new = (1+step_size)*W0_guess;
    end

    % Try to stop it from going negtive and taking too large a step
    if(W0_new > (1 + step_size)*W0_guess)
        W0_new = (1 + step_size)*W0_guess;
    elseif(W0_new < W0_guess/(1 + step_size))
        W0_new = W0_guess/(1 + step_size);
    end
...
S = W0/W_TO_O_S;
fprintf("\nW0 = %.4f kN (%.3f lb), S = %.4f, Wingspan = %.3f", W0/1000, N2lb(W0), S, sqrt(S*AR));

N = 100;
W0_range = linspace(W0*0.5, W0*2,N);
W0_new_data = zeros([1 N]);

for i = 1:N
    W0_new_data(i) = W0_next(W0_range(i), mission, plane, W_TO_O_S, false);
end

plot(W0_range, W0_new_data - W0_range);
xlabel("W0 IN");
ylabel("W0 OUT");

% W0_next(614226, mission, plane, W_TO_O_S, debug)