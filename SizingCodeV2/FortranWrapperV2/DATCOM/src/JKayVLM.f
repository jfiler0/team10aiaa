...
c     file JKayVLM.f - last original verison
...
c	mods by L.T Leifsson, Feb. 2004:
c		- program writes to screen and output file a header
c		  with information about the program and mods
c		- program pauses when run is finished, asks to press
c		  enter to close program
c		

      REAL  VCref(5,10,6,3),DCL(5,2),    DCM(5,2),      QBAR,        
     +      DELslat(5),     DELflap(5),  VC(5,10,6,3),  GAMMA(5,8,10),
     +      FSECT,          Xcg,         Ycg,           Zcg,
     +      LIFT0,          PITCH0,      PI,            LIFT,
     +      PITCH,          ROLL,        YAW,           p,
     +      q,              V,           S,             C,
     +      ALPHA,          LCSDW,       DRAG,          YAW0,
     +      CLalpha,        CmCL,        CLq,           Cmq,
     +      CROLLp,         B,           ASYM,          ASYMDEFL,
     +      DCroll(5,2),    CLATAIL1,    CLATAIL2,      CYAWp,
     +      Cdi,            CdiCL2,      CL0,           Cm0,
     +      Cmalpha,        cref,        Xcgref,        CL,
     +      CH,             Cybeta,      Cnbeta,        Clbeta,
     +      Cyr,            Cnr,         Clr,           BETA,
     +      DCy(5,2),       DCn(5,2),    LIFTTAIL,      LIFTAIL0,
     +      MACH,           bplon(5,4,3),bplat(5,4,3),  slatlon(5),
     +      slatlat(5),     flaplon(5),  flaplat(5),    GAMMAref(5,8,10) 

      INTEGER I,      J,      K,     LAT,   IW1,  IW2,  IWING1,  IWING2,
     +        ITAIL1, ITAIL2, ICASE, ISECT, jsect
...
c     *** Get basic geometric information ***
	...
	write(6,3003)
      ...
      read (*,'(a8)') vlmout
      open(12,file=vlmout)
C      open(UNIT=12,FILE=vlmout,STATUS='NEW',ACTION='WRITE')
      rewind(12)
...
c     *** Ask for longitudinal or lateral case ***
	    ...
	    write(*,*)'Enter Longitudinal Geometry File:'
        read(*,1100) loninf
...
C	LOTUS is an obsolte program!
C      write(*,*) 'Ouput LOTUS 1-2-3 file (y/n)?'
C      read(*,'(a1)') lotus
	lotus = 'n'
...
c     *** Write conditions at top of output ***
 ...
 3000 format('  **************************************************'/
     ...
     2		'  **************************************************'/)
 3001 format('                   Program JKayVLM'/
     1		/'  A program to estimate the subsonic aerodynamics and'/
     2        '  stability and control derivatives for an assessment'/
     3		'  of the control power required in conceptual aircraft'/
     4		'  designs. The basic method is the vortex ring'/
     5		'  variation of the vlm method.'/)
 3002 format('  Conceived and written by Jacob Kay.'/
     1			'  This program modified by'/
     2        '    - W.H. Mason and Alex Benoliel, March 1993'/
     3        '    - W.H. Mason, Nov. 16 1996'/
     4		'    - L.T. Leifsson, Feb 2004'/)
...
*    to conform to the codes coordinate common coordinate system
 ...
 1000 FORMAT(3F10.5)
 1005 format(/,f4.1,/,4(f10.2,/),i1,/,f10.3,/,f10.3)
 1010 format(3(i1,/),i1)
 1100 format(a12)
 1500 format(/3x,'reading longitudinal planform data, isect = ', i2)
 1505 format(/,3x,' Point No.     x(',i1,')',8x,'y(',i1,')',
     &        8x,'z(',i1,')')
 1510 format( 5x,i3,3x,3f12.5)
 1520 format(3x,2x,'slat(',i1,') = ',f9.5,2x,'flap(',i1,') = ',f9.5)
...
c         that corresponds to the wing's sections (IWING1<=IWING2)***
      ...
      CALL DELZERO(ISECT,DELslat,DELflap)
     ...
     +             bplon,slatlon,flaplon)

      Cdi            = 2.*DRAG/(QBAR*S)
      CL0            = 2.*LIFT0/(QBAR*S)
      CM0            = 2.*PITCH0/(QBAR*S*C)
...
c     *** CH changed to reflect that of CG's height above ground ***
      ...
      CALL CENTRAL(ISECT,DELslat,DELflap,ALPHA,CH+Xcg*sin(alpha),q,
     +             p,Xcg,Ycg,Zcg,V,ASYM,ASYMDEFL,LAT,VCref,GAMMAref,
     +             ITAIL1,ITAIL2,IWING1,IWING2,
     +             LIFT,DRAG,PITCH,ROLL,YAW,LIFTTAIL,VC,GAMMA,
     +             bplon,slatlon,flaplon)
c
      CLalpha        = 2.*(LIFT-LIFT0)/(QBAR*S)/ALPHA
      Cmalpha        = 2.*(PITCH-PITCH0)/(QBAR*S*c)/alpha
 ...
 1410 format( 3x,'CL-alpha            = ', f12.5,
     ...
     5       /3x,'Cdi(at alpha = 5)   = ', f12.5)
      ...
      if(itail1.eq.6.and.itail2.eq.6) goto 70
c     
c     *** Calc. lift-curve slope of tail in downwash ***

      CLATAIL1 = 2.0*(LIFTTAIL-LIFTAIL0)/(QBAR*S)/ALPHA
...
c     *** Determine lift-curve slope of tail w/o downwash ***
      CLATAIL2 = 2.0*(LIFTTAIL-LIFTAIL0)/(QBAR*S)/ALPHA
...
c
            CALL DELZERO(ISECT,DELslat,DELflap)
            IF (J .EQ. 1) DELslat(I) = 5.0*PI/180.0
            IF (J .EQ. 2) DELflap(I) = 5.0*PI/180.0
            CALL CENTRAL(ISECT,DELslat,DELflap,ALPHA,CH,q,p,
     +                   Xcg,Ycg,Zcg,V,ASYM,ASYMDEFL,LAT,VCref,GAMMAref,
     +                   ITAIL1,ITAIL2,IWING1,IWING2,
     +                   LIFT,DRAG,PITCH,ROLL,YAW,LIFTTAIL,VC,GAMMA,      
     +                   bplon,slatlon,flaplon)
            DCL(I,J) = 2*(LIFT-LIFT0)/(QBAR*S)/(5.0*PI/180.0)
            DCM(I,J) = 2*(PITCH-PITCH0)/(QBAR*S*C)/(5.0*PI/180.0)   
...
c     *** Multiplied by 2 for the other half of configuration ***
      CROLLp = 2.0*ROLL/(p*(B/2/V))/(QBAR*S*B)
      CYAWp  = 2.0*YAW/(p*(B/BETA/2/V))/(QBAR*S*B)
...
C
     ...
     +             bplon,slatlon,flaplon)
            write(6,*) 'yaw = ', yaw
            DCn(I,J) = 2.0*YAW/(5.0*PI/180.0)/(QBAR*S*B/BETA)      
            DCroll(I,J) = -2.0*ROLL/(5.0*PI/180.0)/(QBAR*S*B)
...
c     *** Get out of Ground effect for Lat/directional derivatives ***
      ...
      ASYMDEFL = 1.0
      LAT      =   1
      CALL DELZERO(JSECT,DELslat,DELflap)
      CALL CENTRAL(JSECT,DELslat,DELflap,ALPHA,CH,q,p,
     +             Xcg,Ycg,Zcg,V,ASYM,ASYMDEFL,LAT,VCref,GAMMAref,
     +             ITAIL1,ITAIL2,IWING1,IWING2,
     +             LIFT,DRAG,PITCH,ROLL,YAW,LIFTTAIL,VC,GAMMA,
     +             bplat,slatlat,flaplat)
...
c
     ...
     +                   LIFT,DRAG,PITCH,ROLL,YAW,LIFTTAIL,VC,GAMMA,      
     +                   bplat,slatlat,flaplat)
            DCy(I,J) = -LIFT/(QBAR*S)/(5.0*PI/180.0)
            DCn(I,J) = -PITCH/(QBAR*S*B/BETA)/(5.0*PI/180.0)
            DCl(I,J) = -ROLL/(QBAR*S*B)/(5.0*PI/180.0)   
 ...
 4000	WRITE(6,3100)
	WRITE(12,3100)
 3100	FORMAT(/'  Run completed successfully.'/)

c     CLOSE(6)  ! removed: closing stdout breaks piped execution
	CLOSE(12)

      READ(*,*) ! was: PAUSE - press enter to close

      STOP
      END

c     *******************************************************************
      SUBROUTINE DELZERO(ISECT,DELslat,DELflap)
      REAL DELslat(5), DELflap(5)
      INTEGER ISECT
c
c     ===================================================================
...
c
 ...
 100  CONTINUE
     ...
     +                   bpin,cslat,cflap)
      ...
      REAL VC(5,10,6,3), BP(4,3), OUT(10,6,3), Cslat(5), Cflap(5)
      REAL Hslat(5,2,3), Hflap(5,2,3), DHslat(2,3), DHflap(2,3)
      REAL DELslat(5), DELflap(5), CP(5,8,5,3),VNOR(5,8,5,4)
      REAL VCref(5,10,6,3),GAMMA(5,8,10),GAMMAref(5,8,10)
      REAL LIFT,PITCH,ROLL,LIFTTAIL,bpin(5,4,3)

      INTEGER  ISECT, IP(5), JP(5), IPslat(5), IPflap(5), LAT
...
c     *** Perform VLM calculation ***
     ...
     +         LIFT,DRAG,PITCH,ROLL,YAW,LIFTTAIL,GAMMA)
C
C
      END
...
c     *******************************************************************
     ...
     +                    Hslat,Hflap,IPslat,IPflap)
      ...
      REAL BP(4,3), Cslat, Cflap, ALPHA, VC(10,6,3), LAMBDA
      REAL Hslat(2,3), Hflap(2,3)
...
c     *** Some constants for the section ***
      ...
      Ct        = BP(3,1)-BP(2,1)
      LAMBDA    = Ct/Cr
      THETAr    = -ASIN((BP(4,3)-BP(1,3))/(BP(4,1)-BP(1,1)))
      THETAt    = -ASIN((BP(3,3)-BP(2,3))/(BP(3,1)-BP(2,1)))
c
c     *** At each spanwise station: ***
      DO 110, K = 1, JP+1
         Y      = DY*(K-1)
         CHORD  = Cr+(Ct-Cr)*Y/B
         THETA  = THETAr+ATAN((LAMBDA*TAN(THETAt-THETAr)*Y/B)/
     +            (1-(1-LAMBDA)*Y/B))
...
c        *** Skip over if no slat ***
 ...
 50      DO 120, J    = 1, IPslat+1
            XINC      = DX*(J-1)
            X         = XLE+XINC+XDIS
            VC(J,K,1) = X*COS(ALPHA)-(ZLE-(X-XLE)*
     +                  TAN(THETA))*SIN(ALPHA)
            VC(J,K,2) = Y+BP(1,2)
            VC(J,K,3) = -X*SIN(ALPHA)+(ZLE-(X-XLE)*
     +                   TAN(THETA))*COS(ALPHA)
 120     CONTINUE
c        *** Skip to find hinge line end pts if all-moving surface ***
         IF (Cslat.EQ.1.0) GO TO 110
c
c
...
c        *** If no flap ***
 ...
 60      DO 130, J = IP+1-IPflap, IP+1
     ...
     +                  TAN(THETA))*SIN(ALPHA)
            VC(J,K,2) = Y+BP(1,2)
            VC(J,K,3) = -X*SIN(ALPHA)+(ZLE-(X-
     +                  XLE)*TAN(THETA))*COS(ALPHA)
 130     CONTINUE
c
...
c     *** One pt going thru c/4 of mean chord ***
 300  Ymac       = B*(1.0+2.0*LAMBDA)/(3.0*(1.0+LAMBDA))+BP(1,2)
      ...
      THETA      = THETAr+ATAN((LAMBDA*TAN(THETAt-THETAr)*Y/B)/
     +             (1-(1-LAMBDA)*Y/B))
      Hslat(2,1) = X*COS(ALPHA)-(ZLE-(X-XLE)*TAN(THETA))*SIN(ALPHA)
      Hslat(2,2) = Ymac
      Hslat(2,3) = -X*SIN(ALPHA)+(ZLE-(X-XLE)*TAN(THETA))*COS(ALPHA) 
c
c     *** The other pt forms a line perpendicular to A/C centerline ***
      ...
      Hslat(1,2)   = BP(1,2)
      THETA        = THETAr
      X            = (ZLE*SIN(ALPHA)+XLE*TAN(THETA)*SIN(ALPHA))/
     +               (COS(ALPHA)+SIN(ALPHA)*TAN(THETA))
      Hslat(1,3)   = -X*SIN(ALPHA)+(ZLE-(X-XLE)*TAN(THETA))*COS(ALPHA)
...
c         be parallel to the vector from A to B ***
      PSI   =  ATAN2(Y0,X0)
      THETA = -ASIN(Z0/SQRT(X0**2+Y0**2+Z0**2))
c
...
c     *******************************************************************
      SUBROUTINE CONPT(QF,QC,DS,IP,JP,ISECT)
      REAL QF(5,10,6,3),QC(5,8,5,3),DS(5,8,5,4)
      INTEGER IP(5), JP(5)
c
c
c     ==================================================================
...
c     *** Control Points ***
     ...
     +                        QF(K,I+1,J+1,1)+QF(K,I+1,J,1))/4.0
               QC(K,I,J,2) = (QF(K,I,J,2)+QF(K,I,J+1,2)+
     +                        QF(K,I+1,J+1,2)+QF(K,I+1,J,2))/4.0
               QC(K,I,J,3) = (QF(K,I,J,3)+QF(K,I,J+1,3)+
     +                        QF(K,I+1,J+1,3)+QF(K,I+1,J,3))/4.0
c
...
c   
      ...
      s22  = e1*zb-xb*e3
      s23  = xb*e2-yb*e1
      s    = .5*(SQRT(s11**2+s12**2+s13**2)+SQRT(s21**2+s22**2+s23**2))
c
c
      RETURN
      END
...
c     ******************************************************************
     ...
     +               IWING1,IWING2,
     +               LIFT,DRAG,PITCH,roll,YAW,LIFTTAIL,GAMMA)
      REAL LIFT,PITCH,ROLL, LIFTTAIL,xmact,zmact

      common /consts/ PI
      DIMENSION QF(5,10,6,3), QC(5,8,5,3), DS(5,8,5,4)
      DIMENSION GAMMA(5,8,10),GAMMAref(5,8,10),QFref(5,10,6,3)
      DIMENSION A(200,200), GAMMA1(200), DW(200), SYM(200)
      DIMENSION GAMMAnew(5,8,10),DELslat(5),DELflap(5)
      DIMENSION Hslat(5,2,3),Hflap(5,2,3),IPslat(5),IPflap(5)
      DIMENSION A1(5,9,5), JP(5), IP(5)
c    
c     ==================================================================
...
c           *** The original half of wing ***
     ...
     +                1.0,M,I,J,DS,ip,jp,CASE,A1,QF,ISECT,
     +                IWING1,IWING2)
c
            L = 0
            DO 125, M1 = 1, ISECT
...
c           *** Reverse flap deflection for antisymmetric deflection ***
            IF (ASYMDEFL .EQ. -1.0) CALL REVERSE(DELslat,DELflap,
...
c           *** Ground effect (mirror image) ***
     ...
     +                u,v,w,1.0,M,I,J,DS,ip,jp,CASE,A1,QF,ISECT,
     +                IWING1,IWING2)
            L = 0
            DO 175, M1 = 1, ISECT
...
c              *** Induced Drag ***
     ...
     +                u1,v1,w1,
     +                0.0,M,I,J,DS,ip,jp,CASE,A1,QF,ISECT,
     +                IWING1,IWING2)
               CALL WING(QC(M,I,J,1),-QC(M,I,J,2),QC(M,I,J,3),GAMMA,
     +                u2,v2,w2,
     +                0.0,M,I,J,DS,ip,jp,CASE,A1,QF,ISECT,
...
c 
c              *** Moments w/r C.G. ***
               xmact   = -((QF(M,I,J,1)+QF(M,I,J+1,1))/2-Xcg*cos(alpha))
               zmact   = -((QF(M,I,J,3)+QF(M,I,J+1,3))/2+Xcg*sin(alpha))
...
c              *** To locate the aerodyanmic center of panel ***
     ...
     +                   (QF(M,I+1,J,1)-QF(M,I,J,1))
               YARM    = QF(M,I,J,2)+(QF(M,I,J+1,2)-QF(M,I,J,2))/3.0*
     +                   ((1.0+2.0*CLAMBDA)/(1.0+CLAMBDA))-Ycg
               ZARM    = (QF(M,I,J+1,3)+QF(M,I,J,3))/2.0-Zcg
               Rmoment = YARM*DLIFT-ZARM*DSIDEFORCE
               XARM    = (QF(M,I,J+1,1)+QF(M,I,J,1))/2.0-Xcg
...
c     ********************************************************************
     ...
     +                DS,IP,JP,CASE,A1,QF,ISECT,IWING1,IWING2)
      DIMENSION GAMMA(5,8,10),QF(5,10,6,3),A1(5,9,5)
      DIMENSION DS(5,8,5,4),IP(5),JP(5)
c
c     ====================================================================
...
c
      COEFF  = GAMMA/(4.0*PI*SQUARE)*(R0R1/R1 - R0R2/R2)
      U      = R1R2X*COEFF
      V      = R1R2Y*COEFF
      W      = R1R2Z*COEFF    
C
...
c     *******************************************************************
      SUBROUTINE MATRIX (N,A,D,C)
      INTEGER N
      DIMENSION A(200,200),C(200),D(200)
...
c     *****************************************************************
     ...
     +                   IPflap,ISECT,IP,JP)
      REAL DELslat(5),DELflap(5),VC(5,10,6,3),Hslat(5,2,3),Hflap(5,2,3)
      INTEGER IPslat(5),IPflap(5),ISECT,IP(5),JP(5)
c
c     ==================================================================
